...
function showCategory(category) {
    // 隐藏所有问题详情
    document.getElementById('loveQuestions').style.display = 'none';
    document.getElementById('wealthQuestions').style.display = 'none';
    document.getElementById('dangerQuestions').style.display = 'none';
    
    // 清除之前的答案
    document.getElementById('loveAnswer').innerHTML = '';
    document.getElementById('wealthAnswer').innerHTML = '';
    document.getElementById('dangerAnswer').innerHTML = '';
    
    // 显示选中的类别
    document.getElementById(category + 'Questions').style.display = 'block';
    document.getElementById(category + 'Questions').scrollIntoView({ behavior: 'smooth' });
}
...
const SPECIFIC_ANSWERS = {
    love: {
        single: {
            title: '🌸 正缘何时来？',
            answers: [
                '八字显示你的正缘将在未来6个月内出现，多参加社交活动，留意身边新认识的朋友。',
                '今年桃花运较旺，特别是在农历七八月份，有机会遇到心仪对象。',
                '你的正缘可能通过工作场合或朋友介绍认识，保持开放的心态。'
            ]
        },
        crush: {
            title: '💌 对方心意如何？',
            answers: [
                '对方对你有好感，但还在观望阶段。建议多创造相处机会，展现你的真诚。',
                '八字显示你们有缘分，但需要你来主动迈出第一步。',
                '目前对方心思在事业上，感情之事不急，慢慢培养感情为上。'
            ]
        },
        breakup: {
            title: '💔 复合可能性',
            answers: [
                '缘分未尽，复合有望。但需要双方都有改变的诚意，建议冷静期后尝试沟通。',
                '复合机会存在，但需要解决导致分手的根本问题，否则容易重蹈覆辙。',
                '八字显示这段感情还有转机，但时机未到，建议先专注自我提升。'
            ]
        },
        marriage: {
            title: '💍 婚姻时机',
            answers: [
                '今年是结婚的好时机，天时地利人和，婚后生活会比较稳定幸福。',
                '可以再磨合半年到一年，感情基础会更稳固，明年结婚更佳。',
                '目前时机尚可，但要注意婚前沟通好重要事项，避免婚后矛盾。'
            ]
        },
        stable: {
            title: '🏠 婚姻稳定性',
            answers: [
                '八字相合，婚姻基础稳固，只要相互包容，白头偕老不成问题。',
                '有小波折但无大碍，遇到问题多沟通，婚姻会越走越稳。',
                '需要注意婚后第三年的考验期，提前做好准备，度过即可长久。'
            ]
        }
    },
    wealth: {
        salary: {
            title: '💵 涨薪前景',
            answers: [
                '今年有升职加薪的机会，特别是农历三四月份，努力工作会被看见。',
                '涨薪需要等待时机，建议先提升核心竞争力，下半年机会更大。',
                '可以考虑主动争取，你的付出值得更好的回报，但方式要得当。'
            ]
        },
        invest: {
            title: '📈 投资建议',
            answers: [
                '近期投资运尚可，但不宜激进，稳健型理财产品更适合你。',
                '投资需谨慎，建议先观望，等局势明朗再入场更安全。',
                '有偏财机会，但记住见好就收，不要贪心，设置好止损点。'
            ]
        },
        job: {
            title: '🔄 跳槽建议',
            answers: [
                '今年适合跳槽，会有更好的平台和发展空间，大胆去争取。',
                '建议先稳住，目前不是最佳时机，可以等明年春天再考虑。',
                '有机会但需慎重，新offer要全面评估，不要只看薪资。'
            ]
        },
        side: {
            title: '💎 副业建议',
            answers: [
                '适合发展副业，特别是与你专业技能相关的领域，会有不错收入。',
                '副业可以尝试，但不要影响主业，时间精力要分配好。',
                '目前副业运势一般，建议先精进主业，等机会成熟再考虑。'
            ]
        },
        windfall: {
            title: '🎰 偏财运',
            answers: [
                '近期偏财运不错，有小财入账，但切记不可赌博，见好就收。',
                '偏财运势平平，不要抱太大希望，踏实工作才是正道。',
                '有意外之财的机会，可能来自奖金、红包或投资回报。'
            ]
        }
    },
    danger: {
        travel: {
            title: '✈️ 出行安全',
            answers: [
                '出行顺利，注意安全即可，旅途会有不错的收获。',
                '近期不宜远行，如有必要出行，注意交通安全，避开危险时段。',
                '出行需谨慎，特别是水上活动要小心，提前做好应急预案。'
            ]
        },
        contract: {
            title: '📝 签约建议',
            answers: [
                '签约顺利，条款对你有利，可以大胆推进。',
                '签约需谨慎，仔细审阅合同条款，特别是违约责任部分。',
                '建议暂缓签约，等待更有利的时机，或请专业人士把关。'
            ]
        },
        health: {
            title: '🏥 健康提示',
            answers: [
                '身体健康，注意作息规律，适当运动即可。',
                '需要注意肠胃和睡眠问题，不要熬夜，饮食要规律。',
                '建议做一次全面体检，特别是心血管方面，防患于未然。'
            ]
        },
        lawsuit: {
            title: '⚖️ 官司口舌',
            answers: [
                '官司能化解，对方态度会软化，通过调解可以和平解决。',
                '需要耐心应对，过程会比较漫长，但最终结果对你有利。',
                '建议寻求专业法律帮助，不要意气用事，以和为贵。'
            ]
        },
        accident: {
            title: '🚗 灾厄提醒',
            answers: [
                '平安无事，保持平常心，但日常安全意识不可松懈。',
                '注意交通安全，特别是驾驶时要专注，不要疲劳驾驶。',
                '有血光之虞，建议捐血或体检化解，出入平安符可保平安。'
            ]
        }
    }
};

function askSpecific(category, question) {
    const data = SPECIFIC_ANSWERS[category][question];
    if (!data) return;
    ...
    // 获取当前运势
    const scores = currentBaziData ? currentBaziData.scores : null;
    let extraAdvice = '';
    
    if (scores) {
        if (scores.overall >= 80) {
            extraAdvice = '<p style="color: #90ee90; margin-top: 15px;"><strong>✨ 结合今日运势：</strong>今日运势较旺，此事可成，宜积极行动。</p>';
        } else if (scores.overall >= 60) {
            extraAdvice = '<p style="color: #f4e5c2; margin-top: 15px;"><strong>✨ 结合今日运势：</strong>今日运势平稳，稳扎稳打，顺其自然。</p>';
        } else {
            extraAdvice = '<p style="color: #ff6b6b; margin-top: 15px;"><strong>⚠️ 结合今日运势：</strong>今日运势平平，建议谨慎行事，暂缓重大决定。</p>';
        }
    }
    
    // 显示答案
    const answerHtml = `
        <h5 style="color: #d4af37; margin-bottom: 10px;">${data.title}</h5>
        <p style="line-height: 1.8;">${answer}</p>
        ${extraAdvice}
        <p style="color: #888; font-size: 0.85em; margin-top: 15px; font-style: italic;">💡 提示：以上仅供参考，命运掌握在自己手中。</p>
    `;
    
    document.getElementById(category + 'Answer').innerHTML = answerHtml;
    document.getElementById(category + 'Answer').style.display = 'block';
    document.getElementById(category + 'Answer').scrollIntoView({ behavior: 'smooth' });
}
...
function displayResults(sizhu, reading, scores, birthYear, birthMonth, birthDay) {
    // 防御性检查
    if (!sizhu || !Array.isArray(sizhu) || sizhu.length === 0) {
        console.error('Invalid sizhu data:', sizhu);
        alert('八字计算出错，请刷新页面重试');
        return;
    }
    if (!reading) {
        console.error('Invalid reading data:', reading);
        alert('详细解读数据缺失，请刷新页面重试');
        return;
    }
    if (!scores) {
        console.error('Invalid scores data:', scores);
        alert('运势评分数据缺失，请刷新页面重试');
        return;
    }
    ...
    if (!sizhuRow) {
        console.error('sizhuRow element not found');
        return;
    }
    sizhuRow.innerHTML = '';
